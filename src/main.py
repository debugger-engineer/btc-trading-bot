...
BINANCE_SYMBOL   = _token.get("binance_symbol", f"{PERPS_SYMBOL}USDT")
...
PERPS_CAPITAL_PCT   = min(_token["capital_pct"], _risk_pct)
...
logger.info(
    "Account: %s | symbol=%s | leverage=%dx | risk=%.1f%% | capital_pct=%.1f%% (cap=%.1f%%) | SL=%.1f%% | BB(%d,%.1f) EMA(%d)",
    ACCOUNT_NAME, PERPS_SYMBOL, PERPS_LEVERAGE, TARGET_RISK_PCT,
    PERPS_CAPITAL_PCT, _token["capital_pct"],
